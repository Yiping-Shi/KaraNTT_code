...
if __name__ == "__main__":
    ...
    # INTT_3D_HW
    print("Computing INTT_3D_HW...")
    start_time = time.time()
    A_intt = INTT_3D_HW(A,psi_inv,q)
    intt_hw_time = time.time() - start_time
    print(f"INTT_3D_HW time: {intt_hw_time:.2f} seconds")
    print("")
    
    # ***************************************
    intt_baseline = load_result_from_file("./INTT_results/INTT_baseline.txt")
    print("INTT_HW vs INTT_BASELINE --> " + ("Correct" if all(x == y for x, y in zip(A_intt, intt_baseline)) 
                           else f"Wrong at position {next(i for i, (x, y) in enumerate(zip(A_intt, intt_baseline)) if x != y)}"))
    # ***************************************