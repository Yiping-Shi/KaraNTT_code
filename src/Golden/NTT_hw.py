...
if __name__ == "__main__":
    ...
    # NTT_3D_HW
    print("Computing NTT_3D_HW...")
    start_time = time.time()
    A_ntt = NTT_3D_HW(A,psi,q)
    ntt_hw_time = time.time() - start_time
    print(f"NTT_3D_HW time: {ntt_hw_time:.2f} seconds")
    print("")
    
    # ***************************************
    ntt_baseline = load_result_from_file("./NTT_results/NTT_baseline.txt")
    print("NTT_HW vs NTT_BASELINE --> " + ("Correct" if all(x == y for x, y in zip(A_ntt, ntt_baseline)) 
                           else f"Wrong at position {next(i for i, (x, y) in enumerate(zip(A_ntt, ntt_baseline)) if x != y)}"))
    # ***************************************