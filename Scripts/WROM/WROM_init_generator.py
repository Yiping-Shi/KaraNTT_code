...
def generate_wrom_files():
    # 确保WROM目录存在
    os.makedirs("WROM_mem_files", exist_ok=True)
    
    # 生成WROM0 (原WROM1)
    print("Generating WROM0 files...")
    for bank in range(16):
        filename = f"WROM_mem_files/wrom0_bank{bank}.txt"
    ...
    for bank in range(16):
        filename = f"WROM_mem_files/wrom1_bank{bank}.txt"
        with open(filename, "w") as f:
            for addr in range(4096):
    ...
    for bank in range(16):
        filename = f"WROM_mem_files/wrom2_bank{bank}.txt"
        with open(filename, "w") as f:
